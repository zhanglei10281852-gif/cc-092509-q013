...
- 法律保全：收到诉讼或监管调查通知后，法务可按项目、来源家族或泄密事件冻结一组档案；保全期间即使原定保存期限到期也禁止处置，支持多次延期与解除，相同案件与范围的重复冻结自动拦截，冻结来源与范围可随时查询。
- 保存期限到期评估：汇总已到期的载体，区分“被保全阻断”与“可进入处置计划”两类结果。
- 处置计划：解除保全后，只有满足密级策略与双人复核条件的载体才能生成计划；计划生成时记录资格判断快照与摘要，保全重新生效或档案版本变化时自动暂停，执行后可核对实际载体、见证人与证明摘要，历史计划与决定只增不改。
