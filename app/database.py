...
CREATE TABLE IF NOT EXISTS legal_holds (
    ...
    source_type TEXT NOT NULL CHECK(source_type IN ('litigation','regulatory_investigation')),
    source_reference TEXT NOT NULL,
...
CREATE INDEX IF NOT EXISTS idx_legal_holds_scope ON legal_holds(scope_type, scope_key, state);
...
CREATE TABLE IF NOT EXISTS legal_hold_extensions (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    hold_id INTEGER NOT NULL REFERENCES legal_holds(id) ON DELETE CASCADE,
    previous_review_by TEXT NOT NULL,
    new_review_by TEXT NOT NULL,
    reason TEXT NOT NULL,
    created_by INTEGER NOT NULL REFERENCES users(id),
    created_at TEXT NOT NULL
);
...
CREATE INDEX IF NOT EXISTS idx_legal_hold_events_hold ON legal_hold_events(hold_id, id);
...
CREATE TABLE IF NOT EXISTS disposal_plans (
    ...
    method TEXT NOT NULL,
    state TEXT NOT NULL CHECK(state IN ('scheduled','paused','executed','cancelled')),
...
CREATE INDEX IF NOT EXISTS idx_disposal_plans_dossier ON disposal_plans(dossier_id, state);
...
CREATE TABLE IF NOT EXISTS retention_assessments (
    ...
    outcome TEXT NOT NULL CHECK(outcome IN ('not_due','held','eligible')),
    active_hold_count INTEGER NOT NULL DEFAULT 0,
    details_json TEXT NOT NULL DEFAULT '{}',
    assessed_by INTEGER NOT NULL REFERENCES users(id)
);
CREATE INDEX IF NOT EXISTS idx_retention_assessments_dossier ON retention_assessments(dossier_id, id);
"""
...
PERMISSIONS = [
    ...
    ("retention.assess", "执行保存期限到期评估", "retention", "assess"),
    ("disposal_plans.manage", "管理处置计划", "disposal_plans", "manage"),
]
...
def _ensure_columns(connection: sqlite3.Connection) -> None:
    """为早期版本创建的档案表补充新增列（CREATE TABLE IF NOT EXISTS 不会更新旧表）。"""
    existing = {row[1] for row in connection.execute("PRAGMA table_info(dossiers)").fetchall()}
    if "secrecy_level" not in existing:
        connection.execute("ALTER TABLE dossiers ADD COLUMN secrecy_level TEXT NOT NULL DEFAULT 'internal'")
    if "retention_until" not in existing:
...
def init_db() -> None:
    ...
    with transaction(immediate=True) as connection:
        ...
        roles = (
            ...
            ("legal_officer", "法务专员", "管理法律保全、保存期限到期评估与处置计划"),
        )
